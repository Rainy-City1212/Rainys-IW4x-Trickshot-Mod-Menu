...
rainyBuildClientsRender()
{
    ...
    {
        self.menuHud0 set_text("   No players found");
        return;
    }
...
rainyForgeModeLoop()
{
    ...
    while (true)
    {
        ...
        if (isDefined(trace["entity"]))
        {
            ...
            {
                self rainyProtectForgedCarePackage(trace["entity"]);
...
rainyIsStockCarePackage(ent)
{
    if (!isDefined(ent))
        return false;

    if (isDefined(ent.targetname) && ent.targetname == "care_package")
        return true;

    if (isDefined(ent.dropType) && isDefined(ent.crateType))
        return true;

    return false;
}
rainyProtectForgedCarePackage(ent)
{
    if (!self rainyIsStockCarePackage(ent))
        return;

    if (isDefined(ent.rainyNoTimeoutStarted) && ent.rainyNoTimeoutStarted)
        return;

    ent.rainyNoTimeoutStarted = true;
    ent thread rainyCarePackageNoTimeoutLoop();
}
rainyCarePackageNoTimeoutLoop()
{
    self endon("death");
    level endon("game_ended");
    level endon("rainy_shutdown");
    ...
    for (;;)
    {
        ...
        // Stock airdrops run their own timeout after landing. That timeout only
        // deletes crates while curProgress is 0, so keep forged packages parked
        // just above idle progress. When a player actually grabs the package,
        // stock useHoldThink owns curProgress/inUse and deleteCrate handles pickup.
